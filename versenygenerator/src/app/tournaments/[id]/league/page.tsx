...
export default function LeagueMatches({ params }: { params: { id: string } }) {
  const id = params.id;
    ...
    async function fetchData() {
      try {
        const [matchRes, refRes] = await Promise.all([
          fetch(`/api/league?tournamentId=${id}`),
  ...
  return (
    <div className="flex flex-col gap-12 items-center w-full">
      <MatchControlPanel
        tournamentId={id}
        generateEndpoint="/api/league"
        generateLabel="Bajnoki mérkőzések generálása"
      />
                    ...
                    <div key={match.match_id} className="mb-8 w-full">
                      <div className="relative flex items-center w-full">
                        <div className="w-full flex flex-col items-center justify-center">
                          ...
                          {result && (
                            <span className="text-2xl font-extrabold mt-2 mb-3 text-center">{result}</span>
                          )}
                        </div>
